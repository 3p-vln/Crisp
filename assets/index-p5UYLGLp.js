import{g as n,L as o,f as l,a as c,B as y,A as v}from"./stop-preload-CWe_wQxZ.js";import{S as p,N as g,A as w,a as $}from"./skeleton-IR_OxhDj.js";import{P as V}from"./pagination-AmZVnSsV.js";import{L as C}from"./btnLoad-BcE4c27Q.js";p.use([g,V,w]);const m=n(".summer-sale__swiper-wrapper");class L{swiper;slidesArr;constructor(){this.swiper=new p(".summer-sale__swiper",{slidesPerView:2,autoplay:{delay:1e4},grabCursor:!0,pagination:{el:".swiper-pagination"},navigation:{nextEl:".swiper-btn-next",prevEl:".swiper-btn-prew"}}),this.slidesArr=[],this.loadCards().then(()=>{o.stop("summer-sale")})}async loadCards(){const e={projectId:"crisp-b06bf"},r={structuredQuery:{from:[{collectionId:"main-swiper"}]}},a=`https://firestore.googleapis.com/v1/projects/${e.projectId}/databases/(default)/documents:runQuery`,t=await l(a,{method:"POST",body:r});if(t.error){console.error("Ошибка при загрузке данных:",t.error);return}t.data&&(t.data.forEach(s=>{this.slidesArr.push(s.document.fields)}),console.log(this.slidesArr),this.renderSlides(),this.swiper.update())}renderSlides(){this.slidesArr.forEach(e=>{m&&m.insertAdjacentHTML("beforeend",`
        ...
        `)})}hidden(){const e=c(".most-popular__swiper-slide");for(let r=0;r<e.length;r++)if(window.innerWidth<1023)if(window.innerWidth>576){if(r<6)continue;e[r].classList.add("hidden")}else{if(r<4)continue;e[r].classList.add("hidden")}}}const b=n(".blog__cards");class I{newsDb;constructor(){this.newsDb=[],this.loadCards().then(()=>o.stop("blog__cards"))}async loadCards(){const e={projectId:"crisp-b06bf"},r={structuredQuery:{from:[{collectionId:"blog"}]}},a=`https://firestore.googleapis.com/v1/projects/${e.projectId}/databases/(default)/documents:runQuery`,t=await l(a,{method:"POST",body:r});if(t.error){console.error("Ошибка при загрузке данных:",t.error);return}t.data&&(t.data.forEach(s=>{if(s.document&&s.document.fields){const i=s.document.name.split("/").pop()||"";this.newsDb.push({id:i,data:s.document.fields})}}),console.log(this.newsDb),this.renderCard())}renderCard(){this.newsDb.forEach(e=>{b&&b.insertAdjacentHTML("afterbegin",`
            ...
            `)})}}document.addEventListener("DOMContentLoaded",async()=>{new y,new v,new L,new E,new A,new S,new j,new I,o.stop("brands__item"),o.stop("sales"),o.stop("limits"),o.stop("explore__content")});new $;
